#Exportar
library(readxl)
base <- read_excel("Pruebas/P20/P20 Dedos análisis.xlsx",
                   sheet = "PulD", col_names = FALSE)
...
if(N/2==1){
  ...
  plot(CP,type="l",col="red",main=expression("Aceleraciones capturadas"),
       xlab="% Ciclo", ylab="Aceleración [m/s2]",
       ylim=c(0, max(CICLOS[[1]][,2])+0.05),lwd=2)
  abline(h=c(-10,-20,-30,0,10,20,30),v=c(10,20,30,40,50,60,70,80,90),col="pink")
  ...
  plot(CICLOS[[1]],type="l",col="green",main=expression("Aceleraciones capturadas"),
       xlab="% Ciclo", ylab="Aceleración [m/s2]",
       ylim=c(min(CP[,2])-max(Des[,2]), max(CP[,2])+max(Des[,2])),lwd=1)
  abline(h=c(-10,-20,-30,0,10,20,30),v=c(10,20,30,40,50,60,70,80,90),col="pink")
  ...
  plot(CICLOS[[1]],type="l",col="green",main=expression("Aceleraciones capturadas"),
       xlab="% Ciclo", ylab="Aceleración [m/s2]",
       ylim=c(min(CP[,2])-max(Des[,2]), max(CP[,2])+max(Des[,2])),lwd=1)
  abline(h=c(-30,-20,-10,0,10,20,30),v=c(10,20,30,40,50,60,70,80,90),col="pink")
  lines(CICLOS[[2]],col="blue",lwd=1)
  lines(CICLOS[[3]],col="black",lwd=1)
  lines(CICLOS[[4]],col="brown",lwd=1)
  lines(CICLOS[[5]],col="yellow",lwd=1)
...
plot(CP,type="l",col="red",main=expression("Promedio y desviación estandar"),
     xlab="% Ciclo", ylab="Aceleración [m/s2]",
     ylim=c(min(CP[,2])-max(Des[,2]), max(CP[,2])+max(Des[,2])),lwd=2)
abline(h=c(-10,-20,-30,0,10,20,30),v=c(10,20,30,40,50,60,70,80,90),col="pink")
...
library(xlsx)
write.xlsx(data.frame(CP,Des), "Pruebas/P20/RESULTADOS.xlsx") 
